  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Bulk Lesson Generation - prominent placement right after upload */}
        <Card className="mt-6 border-primary/30 bg-primary/5">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="overwrite-lessons-top" checked={lessonOverwrite} onChange={(e) => setLessonOverwrite(e.target.checked)} className="rounded border-border" />
              <Label htmlFor="overwrite-lessons-top" className="cursor-pointer text-sm">Overwrite existing lessons (regenerate all)</Label>
            </div>
            {isGeneratingLessons && (
              <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${lessonGenProgress.total > 0 ? (lessonGenProgress.current / lessonGenProgress.total) * 100 : 0}%` }} />
                </div>
              </div>
            )}
            {lessonGenResults.length > 0 && (
              <div className="max-h-48 overflow-y-auto space-y-1 text-xs">
                {lessonGenResults.filter(r => r.topic !== 'RATE_LIMITED').map((r, i) => (
                  <div key={i} className={`flex items-center gap-2 px-2 py-1 rounded ${r.status === 'generated' ? 'bg-success/10 text-success' : r.status === 'exists' ? 'bg-muted text-muted-foreground' : 'bg-destructive/10 text-destructive'}`}>
                    {r.status === 'generated' ? <Sparkles className="w-3 h-3" /> : r.status === 'exists' ? <CheckCircle className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
            ...
            <div className="flex gap-2">
              <Button onClick={handleBulkGenerateLessons} disabled={isGeneratingLessons} className="gap-2">
                {isGeneratingLessons ? (<><Loader2 className="w-4 h-4 animate-spin" /> Generating...</>) : (<><BookOpen className="w-4 h-4" /> Generate All Lessons</>)}
              </Button>
              {isGeneratingLessons && (
        ...
        {/* Old bulk lesson generation moved up */}
